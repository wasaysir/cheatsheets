...
\ Input Embedding: Input tokens are mapped to dense vector representations using an embedding layer. $W_0 in RR^(N_"dict" times d)$ 

$N_"dict"$: Learnable lookup table, where embeddings learned iteratively.
...
Typically these values are founda s linear projections of input: $Q = X W_q, K = X W_k, V = X W_v$

When computing attention over mini-batches of $n$ input vectors, we attention-weighted output is: $"softmax"((Q K^T)/sqrt(d))V in RR^(n times d_v)$ where softmax is applied row-wise, for parallelism. (We scale by $sqrt(d)$ to reduce exploding gradients.)
...
To avoid vanishing gradients in deep networks, residual connections allow gradients to bypass layers. Ex: $f_l'(x) = F_l(x) + x$ allows the network to learn a residual correction instead of a full transformation, creating stable optimization and deeper networks.
