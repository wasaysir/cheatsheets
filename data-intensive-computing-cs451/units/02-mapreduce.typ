...
- combine: $(k_2, "List"[v_2]) arrow "List"[(k_2, v_2)]$ Output key must be same type as input key, because this is optionally run before shuffle. Done if mapper is idle. Does same thing as reducer, but before outputting values, to minimize time wasted. Must be commutative AND associative.

After data is split into blocks and given to mappers, they're partitioned and "shuffled" (dealt) to reducers to aggregate value.

#image("../assets/map-reduce-simple.png")

== System View
#image("../assets/map-reduce-physical.png")
Reducers are woken after majority of mapers are done. They need all the ifiles to be complete before they can start. 
